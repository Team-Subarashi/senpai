      ...
      {token ? 
        <OTSession apiKey={apiKey} sessionId={lesson.vonageSessionId} token={token}>
          <div className={classes.streamsContainer}>
            <div>
              <OTPublisher ref={publisher} properties={{ publishVideo: cameraState, publishAudio: micState, showControls: true, style: {buttonDisplayMode: "off"} }} />
              <div className={classes.publisherContainer}>
                <IconButton onClick={toggleVideo} className={classes.streamButton}>
                  {cameraState ? <Videocam /> : <VideocamOff /> }
                </IconButton>
                <IconButton onClick={toggleMic} className={classes.streamButton}>
                  {micState ? <Mic /> : <MicOff /> }
                </IconButton>
              </div>
            </div>
